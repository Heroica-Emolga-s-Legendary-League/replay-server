...
export class ReplayService {
  private readonly replayDirPath = path.resolve(
    process.cwd(),
    'data',
    'replays',
  );
  ...
  async createReplay(newReplay: NewReplayDto) {
    ...
    try {
      await fs.promises.writeFile(
        path.join(this.replayDirPath, `${newReplay.id}.json`),
        JSON.stringify(newReplay),
        { flag: 'wx' },
      );
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === 'EEXIST') {
        throw new ConflictException(
          `Replay with id '${newReplay.id}' already exists`,
        );
      }
  ...
  async getReplay(id: string): Promise<NewReplayDto | null> {
    try {
      const data = await fs.promises.readFile(
        path.join(this.replayDirPath, `${id}.json`),
        'utf-8',
      );
  ...
  async getReplays() {
    ...
    for (const file of files) {
      const data = await fs.promises.readFile(
        path.join(this.replayDirPath, file),
        'utf-8',
      );
